...
import { ArrowUpRight, Link2, RotateCcw, Settings2 } from "lucide-react";
...
const OptionsPnl = () => {
  ...
  return (
    <ToolLayout
      ...
      intro={
        <>
          Build a strategy leg by leg and see the exact payoff at expiry, the
          modelled P&amp;L before expiry, breakevens, and aggregate Greeks.
          Everything runs in your browser — no signup, no data leaves the page,
          and the URL carries the whole position.
        </>
      }
    >
      <div>
      ...
      {/* Stacked on a phone: the market-assumptions button carries a line of
          numbers that has nowhere to wrap next to a 9rem input. */}
      <div className="grid gap-4 sm:flex sm:flex-wrap sm:items-end">
        <div className="sm:w-36">
          <Field
            label={
              position.underlying === "future" ? "Futures price" : "Spot price"
            }
          >
            <NumberField
        ...
        <Popover>
          <PopoverTrigger asChild>
            <Button
              type="button"
              variant="outline"
              className="min-h-10 w-full justify-start gap-2 font-normal sm:w-auto"
            >
      ...
      <div className="border-border/60 bg-background rounded-lg border p-3 sm:p-5">
        <Suspense
          fallback={
            <div className="text-muted-foreground flex h-[18rem] items-center justify-center text-sm sm:h-[26rem]">
              Loading chart…
            </div>
          }
        >
      ...
      >
        <span>
          ...
          <span className="text-muted-foreground mt-1 block text-sm leading-relaxed">
            Fintecy connects your brokers read-only and runs the same maths on
            what you actually hold — live Greeks, realised P&amp;L and payoff
            across every account. Private beta.
          </span>
        </span>
      ...
      <ToolNotes>
        <p>
          <strong className="text-foreground">Method.</strong> The expiry line is
          exact intrinsic value, so its kinks sit precisely on the strikes. The
          pre-expiry line and the Greeks come from Black-Scholes-Merton with a
          continuous dividend yield for spot underlyings, and Black-76 for
          futures. European exercise, one volatility across all legs, quantities
          in contracts × multiplier.
        </p>
        <p>
          Vega is quoted per one volatility point, theta per calendar day.
          Positions are kept in this browser only, and the share link encodes
          them in the URL rather than on a server.
        </p>
        <p className="text-xs">
