...
import { fitSmile, type Quote } from "@/lib/options/smile";
          ...
          <div className="container mx-auto px-4">
            ...
            <div className="max-w-2xl">
              ...
              <p className="text-muted-foreground mt-4 leading-relaxed">
                Put in the bid and ask for a strip of strikes and the tool backs
                out an implied volatility for each side, fits a smile through
                them, and shows where the market disagrees with its own curve.
                Loaded with a worked example — overwrite it with your own board.
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="section py-10 sm:py-14">
          <div className="container mx-auto max-w-5xl px-4">
            ...
            {fit ? (
              ...
              <p className="border-border/60 text-muted-foreground mt-8 rounded-lg border border-dashed p-6 text-center text-sm">
                Three usable quotes are needed before a smile can be fitted. A
                curve through two points is a straight line pretending
                otherwise.
              </p>
            )}
                    ...
                    return (
                      <tr key={index} className="border-border/40 border-b">
                        <td className="py-2 pr-2 w-24">
                          <NumberField
                            value={quote.strike}
                            onChange={(strike) => update(index, { strike })}
                        ...
                        <td className="py-2 text-right tabular-nums">
                          {vol(point?.band.bid ?? null)}
                        </td>
                        <td className="py-2 text-right tabular-nums">
                          {vol(point?.band.ask ?? null)}
                        </td>
                        <td className="py-2 text-right tabular-nums">
                          {vol(point?.fitted ?? null)}
                        </td>
                        <td className="py-2 text-right tabular-nums">
                          {point?.problem ? (
              ...
              <Button
                ...
                onClick={() =>
                  setQuotes((prev) => {
                    ...
                    return [
                      ...prev,
                      { strike, type: "call", bid: 0.05, ask: 0.1 },
                    ];
                  })
                }
              >
            ...
            {flagged.length > 0 ? (
              <p className="border-border/60 bg-accent/30 mt-6 rounded-lg border p-4 text-sm leading-relaxed">
                <strong className="text-foreground">
                  {flagged.length} strike{flagged.length > 1 ? "s" : ""} priced
                  clear of the curve.
                </strong>{" "}
                {flagged
                  .map(
                  ...
                  .join(", ")}
                . The whole bid-ask band sits on one side of the fit, so this is
                not a rounding artefact — which is the beginning of a question,
            ...
            <div className="border-border/40 text-muted-foreground mt-10 space-y-3 border-t pt-8 text-sm leading-relaxed">
              <p>
                <strong className="text-foreground">Method.</strong> Each side
                of each quote is inverted through the same Black-Scholes as the{" "}
                <Link
                ...
                , by Newton's method kept inside a bisection bracket. The curve
                is a quadratic in log-moneyness against the forward,{" "}
                <code>σ(k) = a + b·k + c·k²</code>, fitted by least squares with
                each quote weighted by the inverse square of its own bid-ask
                width in volatility points. The three coefficients are reported
                above as level, skew and curvature.
              </p>
              <p>
                A quadratic rather than SVI on purpose: five parameters fitted to
                eight strikes is unstable, and a curve that jumps around when you
                change one quote teaches nothing. Quotes that cannot imply a
                volatility — no bid, a crossed market, or an option so deep in
                the money that its price carries no vega — are named in the table
                and left out of the fit rather than silently dropped.
              </p>
              <p>
                Rich and cheap are measured against this curve, not against
                value. A strike flagged here is one the fit disagrees with; the
                fit is a three-parameter guess drawn through a handful of points,
                and it is wrong more often than the market is. Read it as a place
                to look, and note that one badly marked quote drags the curve for
                every other strike.
              </p>
              <p className="text-xs">
                For education and analysis. Not investment advice, not a
                recommendation to trade, and not a substitute for your own
                pricing.
              </p>
            </div>
          </div>
