...
const Retirement = () => {
  ...
  return (
    <ToolLayout
      ...
      intro={
        <>
          Build a pot across an ISA, a LISA and a pension, then spend it down and
          see how often the money lasts. A thousand return paths rather than one
          average, because the order the good and bad years arrive in decides the
          answer. Everything is in today&apos;s money and runs in your browser.
        </>
      }
    >
      {/* The three answers, before any of the inputs that produce them. */}
      <div className="grid gap-4 sm:grid-cols-3">
        <div className="border-border/60 bg-background rounded-lg border p-4 sm:p-5">
          <p className="text-muted-foreground text-xs uppercase tracking-wider">
            Chance the money lasts to {s.planToAge}
          </p>
          <p
            className={`mt-1 text-3xl font-semibold tabular-nums sm:text-4xl ${tone}`}
          >
        ...
        <div className="border-border/60 bg-background rounded-lg border p-4 sm:p-5">
          ...
          <p className="text-muted-foreground mt-2 text-sm leading-relaxed">
            A year, at {(TARGET_CONFIDENCE * 100).toFixed(0)}% confidence. You
            are planning to spend {money.format(s.spending)}.
          </p>
        </div>
        <div className="border-border/60 bg-background rounded-lg border p-4 sm:p-5">
          <p className="text-muted-foreground text-xs uppercase tracking-wider">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-1 text-xs leading-relaxed">
          Line is the median path. The darker band holds the middle half of
          outcomes, the lighter one the tenth to the ninetieth percentile.
          {tailClipped
      ...
      <ToolNotes>
        <p>
          <strong className="text-foreground">Method.</strong> A thousand paths of
          annual real returns, drawn lognormally around the mean you set. Real
          means after inflation, so the spending figure you type keeps the same
          standard of living for the whole plan and there is no second inflation
          assumption compounding against the first — which is why the numbers here
          look smaller, and truer, than a calculator that shows you a nominal pot.
        </p>
        <p>
          Withdrawals come from the ISA first, then the LISA once it unlocks at{" "}
          {LISA_ACCESS_AGE}, and the pension last from {PENSION_ACCESS_AGE},
          because a quarter of the pension is tax-free and the rest is charged as
          income at{" "}
          <Link to="/tools/take-home" className="text-primary hover:underline">
            the usual bands
          </Link>
          . That order is worth more than most people expect, and it is why a plan
          can be well funded in total and still fail: retire before{" "}
          {PENSION_ACCESS_AGE} and only the ISA can carry you.
        </p>
        <p>
          Returns are drawn independently each year. Real markets are not quite
          that well behaved — crashes cluster and valuations mean revert — so treat
          the tails as optimistic rather than conservative. State pension and any
          defined-benefit income are treated as guaranteed and inflation-linked.
        </p>
        <p className="text-xs">
          For planning, not advice. I am not a financial adviser, and a model
          that agrees with you is not a reason to act.
        </p>
      </ToolNotes>
    </ToolLayout>
  );
